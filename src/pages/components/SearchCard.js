...
function SearchCard(props) {
  ...
  return (
    <div className=' md:h-[250px] h-[450px] w-[350px] md:w-[650px] lg:w-[850px]  shadow-blue-800 shadow-xl rounded-lg  bg-white bg-opacity-25 gap-2 flex flex-col md:flex-row items-center  '>
        ...
        <div className='flex flex-col ml-4 items-start gap-3  '> 
            <h2 className='text-lg md:text-2xl text-clip  '>{props.title}</h2>
            <h2>Date</h2>
            <div className='flex flex-row items-center gap-3 '>
                <div className='h-[40px] w-[40px] bg-black rounded-full'></div>
                <div>{props.channelTitle}</div>
            </div>
            <h2 className='text-sm text-clip overflow-hidden mb-2'>{props.description}</h2>
        </div>
    </div>
  )
}
